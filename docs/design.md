...
### freee-mcp-soloとのパターン対応

| freee-mcp-solo | smallright | 説明 |
|---|---|---|
| `src/index.ts` (StdioServerTransport) | 同一パターン | エントリポイント |
| `src/server.ts` (createMcpServer + wrap) | 同一パターン | サーバー生成・ツール登録 |
| `src/tools/xxx.ts` (export async function) | 同一パターン | 1ツール1ファイル |
| `FreeeClient` (共有クライアント) | `BrowserManager` (共有ブラウザ) | 状態を持つシングルトン |
| `MasterCache` (共有キャッシュ) | `ZoneManager` / `ElementRegistry` | 共有状態管理 |
...
- 対象ファイル:
  - `C:/Users/yuuki_okubo/ai-management/.mcp.json` （既存ファイルに追記）
...
- 変更内容:
  - `.mcp.json` に以下を追加:
    ...
    "smallright": {
      "type": "stdio",
      "command": "node",
      "args": ["C:/Users/yuuki_okubo/dev/smallright/dist/index.js"]
    }
    ```
